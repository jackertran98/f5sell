...
import ListOrdered from "../listorder";
import { getListOrder, getListOrderStore } from "../../../service/order";
...
class ItemOrder extends Component {
      ...
      this.setState(
        ...
        () => {
          getListOrder({
            ...
            START_TIME: dayStart,
            END_TIME: dayEnd,
            USER_CTV: authUser.GROUPS === "3" ? "" : username,
            STATUS: statusOrser,
            PAGE: this.offset,
            NUMOFPAGE: 8,
            IDSHOP: "BABU12",
  ...
  searchOrder = () => {
    ...
    this.setState({ loadingSlow: true }, () => {
      getListOrder({
        ...
        START_TIME: dayStart,
        END_TIME: dayEnd,
        USER_CTV: authUser.GROUPS === "3" ? picked.USERNAME : username,
        STATUS: statusOrser,
        PAGE: this.offset,
        NUMOFPAGE: 8,
        IDSHOP: "BABU12",
  ...
  onRefresh = () => {
    ...
    getListOrder({
      ...
      .then((result) => {
        ...
        if (result.data.ERROR == "0000") {
          this.setState(
            { data: result.data.INFO, picked: { USERNAME: "" } },
            async () => {
  ...
  handlePicked = (item) => {
    this.setState({ picked: item }, () =>
      this.props.navigation.navigate("OrderUser")
    );
  };
  componentDidMount() {
